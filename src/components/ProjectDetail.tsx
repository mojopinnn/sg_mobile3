...
export default function ProjectDetail({ projectId, onNavigate }: ProjectDetailProps) {
  ...
  const [versionChatMessages, setVersionChatMessages] = useState<Array<{ sender: "user" | "gemini"; text: string; timestamp: string }>>([]);
  ...
  const [versionAnalysisMode, setVersionAnalysisMode] = useState<"general" | "clipping" | "tracking" | "lighting">("general");
  ...
  const handleOpenVersionChat = (ver: Version) => {
    ...
    setVersionChatMessages([
      {
        sender: "gemini",
        text: `안녕하세요! 톰슨 VFX 연구소 제미나이 영상 공정 파트너입니다. \`${ver.code}\` 버전에 수록된 주소 \`${getVersionVideoUrl(ver)}\` 영상을 분석할 준비가 완료되었습니다. 완성도 검수를 원하는 부분을 아래 옵션 칩에서 선택하거나 직접 입력해 주세요!`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }
    ]);
    setVersionAnalysisMode("general");
  };

  const handleSendVersionChatMessage = async () => {
    if (!currentVersionMessage.trim() || !selectedChatVersion) return;
    
    const userMsg = currentVersionMessage;
    const matchedShot = shots.find(s => 
      (selectedChatVersion.entity && selectedChatVersion.entity.id === s.id) || 
      (selectedChatVersion.entity && selectedChatVersion.entity.name === s.code)
    );
    const shotCode = matchedShot ? matchedShot.code : "Unknown Shot";
    const videoUrl = getVersionVideoUrl(selectedChatVersion);
    ...
    try {
      ...
      const res = await fetch("/api/intelligence/analyze-version", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          versionCode: selectedChatVersion.code,
          shotCode: shotCode,
          videoUrl: videoUrl,
          userMessage: userMsg,
          ...
          <div className="bg-white border border-stone-200 rounded-3xl p-5 md:p-6 shadow-2xl relative w-full max-w-2xl flex flex-col h-[640px] max-h-[90vh]">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-stone-400 font-bold mt-1">
                    버전: {selectedChatVersion.code} • 샷 디폴트 프리뷰
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="bg-stone-50 border border-stone-150 rounded-2xl p-3 mb-4 shrink-0 flex items-center justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <span className="text-xs font-black text-stone-800 block truncate">{selectedChatVersion.code}</span>
                <span className="text-[9px] text-stone-500 block truncate mt-0.5 max-w-sm">
                  URL: {getVersionVideoUrl(selectedChatVersion)}
                </span>
              </div>
              <div 
                onClick={() => handlePlayVersionVideo(selectedChatVersion)}
              ...
              >
                {selectedChatVersion.image ? (
                  <>
                    <img
                      src={selectedChatVersion.image}
                      alt={selectedChatVersion.code}
                      className="object-cover w-full h-full group-hover:scale-105 transition"
                      referrerPolicy="no-referrer"
                    />
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (item.mode === "clipping") {
                          promptText = "이 버전 영상의 불꽃 및 광량 폭발 구간에 화이트 클리핑이 생기어 텍스처 디테일이 뭉개지거나 날라가는지 전용 진단을 돌려서 노펙 커브 수정안을 제안해주세요.";
                        } else if (item.mode === "tracking") {
                          promptText = "매치무브와 카메라 핸드헬드 플레이트 상에서 모션 포인트 밀림(Tracking slip)이 있는지 분석해 타격 시 쉐이크 보강 솔루션을 조언해 주십시오.";
                        } else if (item.mode === "lighting") {
                          promptText = "야간 전투 배경 씬의 조도 대치 상황에서 아티스트 감마 블랙 레벨이 완전히 뭉개져 묻히지 않도록 최적 가중 조명 제어를 제시해주세요.";
                        } else {
                          promptText = "이 버전 영상의 콤프 슛 경계면 완만화 에지 마크 랩(light wrap)과 종합 비주얼 합성 품질에 완성도를 체크리스트 요약과 함께 종합 조언 바랍니다.";
                        }
                    ...
                    >
                      ...
                      <span className={`text-[8px] font-medium leading-none block whitespace-nowrap overflow-hidden text-ellipsis ${isActive ? "text-indigo-100" : "text-stone-400"}`}>
                        {item.desc}
                      </span>
                    </button>
                  );
            ...
            <div className="flex-1 overflow-y-auto bg-stone-50 border border-stone-150 rounded-2xl p-4 mb-4 space-y-3 flex flex-col-reverse">
              {/* Using flex-col-reverse with reversed message order inside mapping allows beautiful natural scroll and pin-to-bottom! */}
                      ...
                      >
                        {isUser ? (
                          ...
                          <div className="whitespace-pre-wrap font-sans space-y-1 prose prose-stone max-w-none">
                            ...
                            {msg.text.split("\n").map((line, lIdx) => {
                              if (line.startsWith("### ")) {
                                return <h4 key={lIdx} className="text-stone-950 font-black text-xs pt-1.5 pb-0.5 border-b border-indigo-100">{line.replace("### ", "")}</h4>;
                              }
                              if (line.startsWith("**") && line.endsWith("**")) {
                                return <p key={lIdx} className="text-stone-900 font-extrabold">{line.replace(/\*\*/g, "")}</p>;
                              }
                              if (line.startsWith("- ")) {
                                return <div key={lIdx} className="flex items-start text-stone-850 pl-1.5"><span className="text-indigo-500 mr-1.5 shrink-0">•</span><span>{line.replace("- ", "")}</span></div>;
                              }
                              return <p key={lIdx} className="text-stone-700 min-h-[0.5rem] leading-normal">{line}</p>;
                            })}
                          </div>
                        )}
                      </div>
